...
function invariants(M, generators, orientation_character)
	...
	if #generators eq 0 then
		return [*[[i] : i in [1..rank]], [[] : i in [1..rank]], [1..rank]*];
	elif #generators eq 1 then //surely this is incorrect if it is generated by a single orientation-reversing element? the basis should not be entirely positive in that case
